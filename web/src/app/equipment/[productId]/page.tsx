          ...
          <div className="p-6 flex flex-col gap-3">
            ...
            <p className="font-mono text-2xl">
              {isDumpster || product.daily_rate === 0
                ? 'Delivered · Quote at reservation'
                : `$${product.daily_rate.toFixed(2)} / day`}
            </p>
            {product.description && (
            ...
            <ul className="font-mono text-sm mt-2 flex flex-col gap-1 border-t-2 border-ind-black/10 pt-3">
              <li>Max rental: {product.max_rental_days} days</li>
              <li>Deposit: 30% of rental subtotal (placed at handover)</li>
              {product.requires_towing_ack && (
                <li className="text-ind-danger font-bold uppercase">
                  Towable — towing acknowledgment required
                </li>
              )}
              {isDumpster && <li>Billing: 30% down (delivered category)</li>}
            </ul>
          </div>
        </div>
