...
// with a live yard-status light in its hub, the placeholder wordmark lockup,
// and primary nav. Only the logo swaps later — the gear + type lockup are LOCKED.
    ...
    <header className="bg-ind-black text-ind-yellow border-b-8 border-ind-yellow sticky top-0 z-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
        <Link href="/" className="flex items-center gap-4" aria-label="Eastern Pro Rentals — home">
          {/* Industrial gear — keystone motif (10s linear spin). The yard-status
              light is overlaid in the hub as a non-spinning sibling so it stays
              fixed while the gear rotates around it. */}
          <span className="relative inline-flex h-10 w-10 shrink-0 items-center justify-center">
            <svg
              ...
              viewBox="0 0 24 24"
              fill="currentColor"
              className="text-ind-yellow animate-[spin_10s_linear_infinite]"
              aria-hidden="true"
            >
            ...
            {/* Yard-online light — green, fixed in the gear hub */}
            <span
              className="absolute h-2.5 w-2.5 rounded-full border border-ind-black bg-green-500 animate-pulse"
              aria-hidden="true"
            />
            <span className="sr-only">{t('nav.yardOnline')}</span>
          </span>
