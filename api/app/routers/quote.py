...
def _load_config(svc) -> dict:
    ...
    if not res.data:
        raise HTTPException(status_code=503, detail={"code": "CONFIG_MISSING", "message": "Config not seeded"})
    return res.data[0]
...
def quote(body: QuoteIn):
    ...
    if days > int(p["max_rental_days"]):
        raise HTTPException(
            status_code=400,
            detail={"code": "MAX_DURATION", "message": f"Rentals are limited to {p['max_rental_days']} days"},
        )
    ...
    if body.fulfillment == "delivery" and not settings_has_distance_key():
        raise HTTPException(
            status_code=422,
            detail={"code": "DELIVERY_UNAVAILABLE", "message": "Delivery pricing isn't available yet — choose pickup."},
        )
